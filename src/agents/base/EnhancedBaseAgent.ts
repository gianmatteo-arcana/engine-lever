/**
 * Enhanced BaseAgent - Template Inheritance Implementation
 * 
 * This implements the BaseAgent template inheritance pattern specified in PR feedback.
 * Extends the existing Agent.ts with template merging capabilities.
 * 
 * CRITICAL: Enforces the UNIVERSAL CONTEXT ENTRY CONTRACT
 * - All agents MUST use the exact schema from base_agent.yaml
 * - Backend and frontend layers MUST understand this schema
 * - BaseAgent implementation strictly enforces schema compliance
 */
...
/**
 * Enhanced BaseAgent Class - Universal Foundation with Template Inheritance
 * 
 * This class implements the template inheritance pattern from PR feedback:
 * 1. Loads base_agent.yaml (universal template)
 * 2. Loads specialized agent config (inherits base)
 * 3. Merges templates for LLM prompt construction
 * 4. Enforces universal schema compliance
 */
export abstract class EnhancedBaseAgent {
  ...
  constructor(specializedConfigPath: string) {
  ...
  /**
   * Load base agent template (universal foundation)
   * This is THE CONTRACT that all agents must follow
   */
  private loadBaseTemplate(): BaseAgentTemplate {
    try {
      ...
      // Return the agent_template section (universal patterns)
  ...
  /**
   * Main execution method - implements inheritance pattern
   * 
   * Merges base + specialized templates for LLM as shown in PR feedback:
  ...
  async execute(request: BaseAgentRequest): Promise<BaseAgentResponse> {
    ...
    // Validate and enforce universal schema
    const validatedResponse = this.enforceUniversalSchema(llmResponse, request);
   ...
   * 
   * This implements the exact pattern from PR feedback:
   * Merges base agent principles with specialized mission and current request
   */
...
# Universal Reasoning Framework (MANDATORY)
...
${base.context_patterns.universal_context_entry_schema}
...
`;
  ...
  /**
   * Enforce Universal Context Entry Schema
   * 
   * CRITICAL: This enforces THE CONTRACT from base_agent.yaml
   * All contextUpdate objects MUST follow the universal schema
   */
  private enforceUniversalSchema(llmResponse: any, request: BaseAgentRequest): BaseAgentResponse {
    ...
    // Create universal context entry following THE CONTRACT
    const universalContextEntry: UniversalContextEntry = {
    ...
    return {
      ...
      confidence: this.validateConfidence(llmResponse.confidence || universalContextEntry.confidence),
  ...
  /**
   * Generate unique entry ID following universal format
   */
